...
import { checkMonitors } from "../functions/check-monitors/resource"; // adjust path to match your folder
...
const schema = a
  .schema({
    Monitor: a
      .model({
        ...
        checkInterval: a.integer().default(1),
  ...
  // 🔑 this is what was missing — grants checkMonitors IAM access to ALL models
  .authorization((allow) => [allow.resource(checkMonitors)]);
